...
namespace HeightFieldLod
{
    ...
    public sealed class HeightFieldLodCompute : MonoBehaviour, ILodSource
    {
        ...
        public RenderTexture NormalTexture => _normalMap;
        public Mesh[] LodMeshes => _lodMeshes;
        public ComputeBuffer[] InstanceBuffers => _instanceBuffers;
        public ComputeBuffer[] ArgsBuffers => _argsBuffers;
        public int LodLevelCount => LodLevels;

        public uint GetInstanceCount(int lod)
        {
            if (_argsCpu == null || lod < 0 || lod >= LodLevels) return 0;
            return _argsCpu[lod][1];
        }
        ...
        public void EnsureUpdated(HeightFieldLayout layout, RenderTexture height)
        {
            if (height == null || layout.TexWidth <= 0) return;
            int frame = Time.frameCount;
            if (_lastUpdateFrame == frame && _height == height
                && _lodMeshes != null
                && _layout.TexWidth == layout.TexWidth
                && _layout.TexHeight == layout.TexHeight)
                return;
